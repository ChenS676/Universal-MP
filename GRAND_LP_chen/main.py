...
def compare_dicts(dict1, dict2):
    ...
    non_overlapping = keys1.symmetric_difference(keys2)
    overlapping_same = {k: dict1[k] for k in keys1 & keys2 if dict1[k] == dict2[k]}
    overlapping_different = {k: (dict1[k], dict2[k]) for k in keys1 & keys2 if dict1[k] != dict2[k]}

    print("Non-overlapping keys:", non_overlapping)
    print("\nOverlapping keys with same values:", overlapping_same)
    print("\nOverlapping keys with different values:", overlapping_different)
    

cfg_file = {
  'Cora': 'yamls/cora/gcn.yaml',
  'Citeseer': 'yamls/cora/gcn.yaml',
  'Pubmed': 'yamls/cora/gcn.yaml',  
  'ogbl-ddi': 'yamls/cora/gcn.yaml',
  'ogbl-citation2': 'yamls/cora/gcn.yaml',
  'ogbl-ppa': 'yamls/cora/gcn.yaml',
  'ogbl-vessel': 'yamls/cora/gcn.yaml'
}
...
def main(cmd_opt):
    ...
    # print loaded data
    for val in data.__dict__['_store']:
      if hasattr(data.__dict__['_store'][val], 'shape'):
        print(val, data.__dict__['_store'][val].shape)

    for k, val in splits.items(): 
      for ik, ival in val.items():
        print(ik, ival.size())
      print('\n')
...
if __name__=='__main__':
    ...
    parser.add_argument('--dataset', type=str, default='Cora',
                        help='Cora, Citeseer, Pubmed, ogbl-ppa, ogbl-ddi, ogbl-citation2, ogbl-vessel')
    ...
    # TODO Bug 2 simplify the code I use default value from https://github.com/ChenS676/graph-neural-pde/blob/main/src/run_GNN.py
    opt = vars(args)
