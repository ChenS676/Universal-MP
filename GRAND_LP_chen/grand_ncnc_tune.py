...
if __name__=='__main__':
    ...
    hyperparameter_search = {
                            # 'hiddim': [64, 256], # Hidden dimension size.
                            # "gnndp": [0.0, 0.2, 0.5], # Dropout ratio for GNN.
                            # "xdp": [0.0, 0.3, 0.7], # Dropout ratios for different parts of GNN.
                            # "tdp": [0.0, 0.2], # Dropout ratios for different parts of GNN.
                            # "gnnedp": [0.0], # Edge dropout ratio for GNN.
                            # "predp": [0.0, 0.05], # Dropout ratio for predictor.
                            # "preedp": [0.0, 0.4], # Edge dropout ratio for predictor.
                            
                            "batch_size": [256, 512, 1024, 2048], 
                            "gnnlr": [0.01, 0.001, 0.0001], # Learning rate for GNN.
                            "prelr": [0.0001]} # Learning rate for predictor.
    ...
    for batch_size, gnnlr, prelr in itertools.product(*hyperparameter_search.values()):
        ...
        args.prelr = prelr
        tune_res = {}            
        tune_res['batch_size'] = batch_size
        tune_res['gnnlr'] = gnnlr
        tune_res['prelr'] = prelr
        
        print(f"tune params: {tune_res}")
        ...
        for run in range(args.runs):
            ...
            wandb.init(project="GRAND_tune", name=name_tag, config=opt)
            ...
            if kill_cnt > 3:
                break
            mvari_str2csv(args.name_tag, tune_res, f'results_grand_gnn/tune_{args.data_name}_lm_mrr.csv')
