...
class Trainer_Heart(Trainer):
    def __init__(self, 
        ...
        # check whether train has negative edges
    ...
    def save_eval_edge_pred(self, h, edge_index):
        ...
        edge_index_list = []
        # sourcery skip: no-loop-in-tests
        for perm  in DataLoader(range(edge_index.size(1)), self.batch_size):
            edge = edge_index[perm].t()
    ...
    def _save_err_heart(self, data: Data, mode):
        ...
        if self.model_name == 'VGAE':
            ...
            z = self.model.encoder(self.data.x, self.data.edge_index)

        pos_pred, pos_edge_index = self.save_eval_edge_pred(z, data['edge'])
        neg_pred, neg_edge_index = self.save_eval_edge_pred(z, data['edge_neg'])
    ...
    def _acc_error_save(self, 
        ...
        data = {
            "edge_index0": torch.cat([pos_edge_index, neg_edge_index], dim=0)[:, 0].cpu(),
            "edge_index1": torch.cat([pos_edge_index, neg_edge_index], dim=0)[:, 1].cpu(),
